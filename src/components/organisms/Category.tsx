...
export default async function FieldOfInterests({ field }: { field: Category }) {
	const articles = await Promise.all(
		field.items.articles
			.slice(0, 3)
			.map((slug) => getFieldItemBySlug({ slug, itemsType: "articles" }))
	);
	const projects = await Promise.all(
		field.items.projects
			.slice(0, 3)
			.map((slug) => getFieldItemBySlug({ slug, itemsType: "projects" }))
	);
			...
			<main>
				...
				{field.items.projects.length > 0 && (
					<Section>
						...
						<CardsList
							items={projects.filter((item) => item !== undefined)}
				...
				{field.items.articles.length > 0 && (
					<Section>
						...
						<CardsList
							items={articles.filter((item) => item !== undefined)}
